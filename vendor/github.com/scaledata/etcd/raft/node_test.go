...
func TestNodeStep(t *testing.T) {
	for i, msgn := range sdraftpb.MessageType_name {
		n := &node{
			propc: make(chan sdraftpb.Message, 1),
...
func TestNodeStepUnblock(t *testing.T) {
	...
	n := &node{
		propc: make(chan sdraftpb.Message),
...
func TestNodePropose(t *testing.T) {
	...
	appendStep := func(r *raft, m sdraftpb.Message) {
		msgs = append(msgs, m)
	}
...
// TestNodeReadIndexToOldLeader ensures that sdraftpb.MsgReadIndex to old leader
