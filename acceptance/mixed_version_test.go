...
func generateProcFile(dirPath string, oldConfigs []KronosConfigOld) (string, error) {
	...
	oldBinPath, err := exec.LookPath(kronosPreUpgrade)
	if err != nil {
		return "", err
	}

	for i, config := range oldConfigs {
		oldCmdFile, err := os.OpenFile(filepath.Join(dirPath, fmt.Sprintf("cmd-kronos-node-%v-old.sh", i)), os.O_TRUNC|os.O_WRONLY|os.O_CREATE, 0755)
		if err != nil {
			return "", err
		}
		defer oldCmdFile.Close()
		newCmdFile, err := os.OpenFile(filepath.Join(dirPath, fmt.Sprintf("cmd-kronos-node-%v-new.sh", i)), os.O_TRUNC|os.O_WRONLY|os.O_CREATE, 0755)
		if err != nil {
			return "", err
		}
		defer newCmdFile.Close()
		...
		newConfig.KronosPath = newBinPath
		err = newTmpl.Execute(newCmdFile, newConfig)
		if err != nil {
			return "", err
		}
		config.KronosPath = oldBinPath
		err = oldTmpl.Execute(oldCmdFile, config)
		if err != nil {
			return "", err
		}
		_, err = file.Write([]byte(fmt.Sprintf("kronos%v: %v\n", config.Index, filepath.Join(dirPath, fmt.Sprintf("cmd-kronos-node-%v.sh", i)))))
		if err != nil {
			return "", err
		}
		err = os.Symlink(filepath.Join(dirPath, fmt.Sprintf("cmd-kronos-node-%v-old.sh", i)), filepath.Join(dirPath, fmt.Sprintf("cmd-kronos-node-%v.sh", i)))
		if err != nil {
			return "", err
		}
	}
...
func TestClusterRollingUpgrade(t *testing.T) {
	...
	tc.Procfile, err = generateProcFile(tmpDir, configs)
	a.NoError(err)
	tc.Fs = fs
	...
	for i := 0; i < 8; i++ {
		...
		a.NoError(os.Remove(filepath.Join(tmpDir, fmt.Sprintf("cmd-kronos-node-%v.sh", i))))
		a.NoError(os.Symlink(filepath.Join(tmpDir, fmt.Sprintf("cmd-kronos-node-%v-new.sh", i)), filepath.Join(tmpDir, fmt.Sprintf("cmd-kronos-node-%v.sh", i))))
...
// This test expects two binaries to be present in the path: kronos-1 and kronos-2
// which are the old and new versions of the binary respectively.
func TestClusterRollingUpgradeAllNodesRestart(t *testing.T) {
	ctx := context.Background()
	fs := afero.NewOsFs()
	a := require.New(t)
	tmpDir, err := afero.TempDir(fs, "", "kronos_test_dir_")
...
func TestClusterDisruptiveUpgrade(t *testing.T) {
	ctx := context.Background()
	fs := afero.NewOsFs()
	a := require.New(t)
	tmpDir, err := afero.TempDir(fs, "", "kronos_test_dir_")
