...
func (s *simpleLogger) Write(entry zapcore.Entry, fields []zapcore.Field) error {
	...
	case zapcore.DebugLevel, zapcore.InfoLevel:
		s.Infof(ctx, "%v - %v", entry.Caller, entry.Message)
	case zapcore.ErrorLevel:
		s.Errorf(ctx, "%v - %v", entry.Caller, entry.Message)
	case zapcore.WarnLevel:
		s.Warningf(ctx, "%v - %v", entry.Caller, entry.Message)
	case zapcore.DPanicLevel, zapcore.PanicLevel, zapcore.FatalLevel:
		s.Fatalf(ctx, "%v - %v", entry.Caller, entry.Message)
	default:
		s.Infof(ctx, "%v - %v", entry.Caller, entry.Message)
	}
	return nil
